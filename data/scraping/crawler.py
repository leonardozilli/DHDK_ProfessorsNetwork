...
def threshold(input1, input2):
    ...
    for prof in publications:
        data_list.append((prof, len(publications[prof]['Publications'])))

    first_25 = list(sorted(data_list, key=lambda x: x[1], reverse=True))[:25]
    return list(zip(*first_25))[0]
...
def identify_authors(input, output_dir):
    ...
    internalAuthorsUniqueList = set()

    for prof in data.values():
        if len(prof['Publications']) >= 82:
            for link in prof['Publications']:
                for author in prof['Publications'][link]['internalAuthor']:
                    internalAuthorsUniqueList.add(author)
